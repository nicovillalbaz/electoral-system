...
const EVENT_CATEGORIES: Record<string, { label: string; types: string[] }> = {
  ...
  VOTES: { label: "Votos", types: ["PERSON_MARKED_VOTED", "DAY_D_STATUS_CHANGED"] },
  ...
  LOGISTICS: { label: "Logistica", types: ["TRANSPORT_REQUESTED", "FINANCIAL_REQUESTED"] },
  CONTACTS: { label: "Contactos", types: ["PERSON_CONTACTED", "PERSON_UPDATED"] },
  TAGS: { label: "Etiquetas", types: ["TAG_ASSIGNED", "TAG_REMOVED"] },
};
...
export default function LogsTab({ stationId }: LogsTabProps) {
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/events?stationId=${stationId}&limit=200`);
  ...
  const handleRevert = async (log: any) => {
    if (!confirm("Revertir esta accion?")) return;
    setRevertingId(log.id);
    try {
      await api.post(`/events/${log.id}/revert`);
      await fetchLogs();
      toast.success("Accion revertida correctamente.");
    } catch (e: any) {
      console.error(e);
      toast.error(e?.response?.data?.error || e?.response?.data?.message || "Error al revertir.");
    } finally {
      setRevertingId(null);
    }
  };
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center flex-wrap gap-3">
        <div>
          ...
          <p className="text-zinc-500 text-sm">Ultimos eventos del PC</p>
        </div>
      ...
      {!loading && filteredLogs.length === 0 && (
        <div className="text-zinc-600 text-sm">No hay eventos{categoryFilter !== "ALL" ? " en esta categoria" : " aun"}.</div>
      )}
            ...
            <div className="flex justify-between items-start gap-4">
              <div>
                ...
                <div className="text-xs text-zinc-500">
                  {new Date(log.created_at || log.timestamp).toLocaleString()} · {log.actor_name || "Sistema"}
                </div>
                {log.station_name && (
              ...
              <button
                ...
                title={!isReversible(log) ? "No reversible" : "Revertir accion"}
              >
